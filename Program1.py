...
pg.display.set_caption("Farmer's Market")
TILE_SIZE = 128
clock = pg.time.Clock()

background = pg.image.load('cloud.png')
def background_sky(background):
    size= pg.transform.scale(background, (1000,400))
...
class Player(pg.sprite.Sprite):
    ...
    def draw(self, screen):
        ...
        draw_speech_bubble(screen, "I want 1 apple and 1 orange ", (255, 255, 255), (0, 0, 0), self.rect.midtop, 25)
...
class NPCOne(pg.sprite.Sprite):
    ...
    def draw(self, screen):
        screen.blit(self.image, self.rect.topleft)
        if self.speaking:
            draw_speech_bubble(screen, "Your total is 45 pesos", (255, 255, 0), (175, 175, 0), self.rect.midtop, 25)
